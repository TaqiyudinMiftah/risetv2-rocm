...
def build_confounder_dict(
    ...
    """
    Build confounder dictionary by clustering context features using K-Means++.

    Args:
        context_features: [N, C] tensor of context feature vectors
        num_confounders: K, number of confounder prototypes
        seed: random seed for reproducibility
    Returns:
        confounder_dict: [K, C] cluster centers
        confounder_prior: [K, 1] cluster frequency proportions
    """
    features_np = context_features.numpy()
    num_samples = features_np.shape[0]

    print(f"Clustering {num_samples} context features into {num_confounders} confounders (K-Means++)...")
